...
After Render creates the service, replace `https://YOUR-RENDER-SERVICE.onrender.com` in the `Debug-Navigator` and `Debug-Destination` build configurations with the real Render URL.
...
curl https://YOUR-RENDER-SERVICE.onrender.com/sessions
curl https://YOUR-RENDER-SERVICE.onrender.com/sessions/SESSION_ID/logs
```

Create a backend smoke session:

```bash
curl -X POST https://YOUR-RENDER-SERVICE.onrender.com/sessions \
...
1. On Phone B, open `Share My Location` and grant location permission.
2. On Phone A, open `Navigate to a Contact` and select `Avery`.
